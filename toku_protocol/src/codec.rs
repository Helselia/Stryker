...
use crate::frames::{
    Error as ErrorFrame, Frame, GoAway, Hello, HelloAck, TokuFrame, Ping, Pong, Push, Request,
    Response,
};
...
impl Encoder for Codec {
    ...
    fn encode(&mut self, item: Self::Item, dst: &mut BytesMut) -> Result<(), Self::Error> {
        match item {
            TokuFrame::Hello(frame) => encode(frame, dst),
            TokuFrame::HelloAck(frame) => encode(frame, dst),
            TokuFrame::Ping(frame) => encode(frame, dst),
            TokuFrame::Pong(frame) => encode(frame, dst),
            TokuFrame::Request(frame) => encode(frame, dst),
            TokuFrame::Response(frame) => encode(frame, dst),
            TokuFrame::Push(frame) => encode(frame, dst),
            TokuFrame::GoAway(frame) => encode(frame, dst),
            TokuFrame::Error(frame) => encode(frame, dst),
        };
        Ok(())
    }
}
...
fn decode<F: Frame + Into<TokuFrame>>(
    codec: &mut Codec,
    buf: &mut BytesMut,
) -> Result<Option<TokuFrame>, Error> {
...
mod tests {
    ...
    fn test_frame_round_trip<F: Into<TokuFrame>>(frame_bytes: &[u8], expected_frame: F) {
