defmodule Toku.Client do
  @moduledoc """
  # A high performance client for the Toku protocol

  This client exposes five public functions, `start_link`, `request`, `push`, `ping` and `close`.
  In toku parlance, `request` is a synchronous call and `push` is asynchrounous. Be careful with push
  ...
  When connecting, a Toku client and server negotiate which compressors and codecs they
  should use to communicate.

  The Toku client comes with several preinstalled codecs and compressors. They are:

  ### Codecs

    * `Toku.Protocol.Codecs.Erlpack`: Uses `:erlang.term_to_binary` to encode terms and `:erlang.binary_to_term` to
       decode. All terms can be represented in `Erlpack`.
    * `Toku.Protocol.Codecs.Json`: Uses the `jiffy` JSON library to encode and decode terms. Not all terms are
       able to be represented in JSON.
    * `Toku.Protocol.Codecs.Msgpack`: Uses `Msgpax` to encode data in MessagePack format. Not all terms can
       be represented in MessagePack.

  ### Compressors

   * `Toku.Protocol.Compressors.NoOp`: The default compressor; does nothing
   * `Toku.Protocol.Compressors.Gzip`: Compresses data via `:zlib.gzip` and decompresses data via `:zlib.gunzip`
  ...
  If Toku can detect the presence of the modules, then the codecs will be enabled.
  ...
  ```elixir
  {:ok, client} = Toku.Client.start_link("localhost", 1234, "/_rpc", toku_opts: [codecs: [Useless]])
  ```
  ...
  ```
  alias Toku.Protocol.Codecs.{Erlpack, Json, Msgpack}
  # Starts a Toku client on port 4450 favoring the Json codec, then the Msgpack codec, then the Erlpack codec.
  {:ok, json_client} = Toku.Client.start_link("localhost", 4450, "/_rpc", toku_opts: [codecs: [Json, Msgpack, Erlpack]])
  ```
  ...
  defmodule State do
    ...
    @type last_ping :: {Toku.Client.sequence(), timestamp}
    @type t :: %__MODULE__{
            port: pos_integer,
            host: charlist,
            tcp_opts: Toku.Client.tcp_opts(),
            recv_timeout: pos_integer,
            toku_path: String.t(),
            buffer: binary,
            packet_buffer: iodata,
            sequence: :go_away | Toku.Client.sequence(),
            ...
            codec: Codec.t(),
            waiters: %{Toku.Client.sequence() => pid},
            last_ping: last_ping
          }
  ...
  alias Toku.Protocol.{Codec, Codecs, Compressor, Compressors, Frames, Parser}
  use Connection
  ...
  @type toku_opt :: {:codecs, [Codec.t()]} | {:compressors, [Compressor.t()]}
  @type toku_opts :: [toku_opt]
  ...
  @doc """
  Creates a connection to a Toku server.
  """
  @spec start_link(String.t(), pos_integer, String.t(), opts) :: {:ok, pid} | {:error, term}
  def start_link(host, port, toku_path, options) do
    Connection.start_link(__MODULE__, {host, port, toku_path, options})
  end

  @doc false
  def init({host, port, toku_path, opts}) do
    tcp_opts =
      opts
    ...
    {toku_opts, _opts} = Keyword.pop(opts, :toku_opts, [])
    ...
    case :gen_tcp.connect(host, port, tcp_opts, connect_timeout) do
      {:ok, sock} ->
        ...
        with {:ok, _sock} <- update_socket_opts(sock),
             ...
             {:ok, state} <- do_toku_connect(state) do
          {:ok, state}
        else
          {:error, _} = error ->
            Logger.error("Toku upgrade failed because of #{inspect(error)}")
            {:stop, error, state}
        end
  ...
  defp upgrade_request(%State{toku_path: toku_path, host: host}) do
    "GET #{toku_path} HTTP/1.1\r\nHost: #{host}\r\nUpgrade: toku\r\nConnection: upgrade\r\n\r\n"
  end
