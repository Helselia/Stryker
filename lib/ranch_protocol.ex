defmodule Toku.RanchProtocol do

  use Toku.Opcodes
  alias Toku.{Protocol, Protocol.Frames}
  alias Toku.Protocol.{Codecs, Compressors}
  require Logger

  @type state :: %__MODULE__{}

  @default_ping_interval 30_000
  @max_seq round(:math.pow(2, 32) - 1)
  ...
  def upgrade(socket_pid, transport, handler, handler_opts) do
    state = %__MODULE__{
      ...
      transport: transport,
      handler: handler,
      handler_supports_push: :erlang.function_exported(handler, :toku_push, 2),
      handler_opts: handler_opts
    }

    Logger.info(
      "[toku] upgrade: address=#{inspect(address(state))} socket_pid=#{inspect(socket_pid)}"
    )
  ...
  def handler_init(
    ...
    case handler.toku_init(transport, handler_opts) do
      {:ok, opts} ->
        state
        |> set_opts(opts)
        |> toku_handshake
  ...
  def toku_handshake(%__MODULE__{} = state) do
    state
    |> schedule_ping()
    |> handler_loop(<<>>)
  end
  ...
  def handler_loop(%{socket_pid: socket_pid, transport: transport} = state, so_far) do
    ...
    receive do
      :send_ping ->
        case ping(state) do
          {:error, reason} ->
            Logger.error("[toku] Sending ping failed because #{inspect(reason)}")
      ...
      {:tcp_closed, ^socket_pid} ->
        Logger.info("[toku] tcp_closed. socket_pid=#{inspect(socket_pid)}")
      ...
      other ->
        Logger.info("[toku] unknown message. message=#{inspect(other)}")
        handler_loop(state, so_far)
    end
  end
  ...
  defp next_seq(%{ next_seq: next_seq } = state) when next_seq >= @max_seq do
    {1, %{state | next_seq: 2}}
  end
  ...
  defp handle_request({:request, _flags, seq, request}, state) do
    decoded = from_wire_format(state, request)

    {:ok, handler_request(state, seq, decoded)}
  end

  defp handle_request({:push, _flags, request}, state) do
    decoded = from_wire_format(state, request)

    {:ok, handler_push(state, decoded)}
  end
  ...
  defp goaway(state, :normal), do: goaway(state, 0, "Normal")

  defp goaway(state, :invalid_op), do: goaway(state, 1, "InvalidOp")

  defp goaway(state, :unsupported_version), do: goaway(state, 2, "UnsupportedVersion")

  defp goaway(state, :no_common_encoding), do: goaway(state, 3, "NoCommonEncoding")

  defp goaway(state, :invalid_encoding), do: goaway(state, 4, "InvalidEncoding")

  defp goaway(state, :invalid_compression), do: goaway(state, 5, "InvalidCompression")

  defp goaway(state, :ping_timeout), do: goaway(state, 6, "PingTimeout")

  defp goaway(state, :internal_server_error), do: goaway(state, 7, "InternalServerError")

  defp goaway(state, :not_enough_options), do: goaway(state, 8, "NotEnoughOptions")

  defp goaway(%{ socket_pid: socket_pid } = state, code, reason) do
    Logger.info(
      "[toku] goaway. address=#{inspect(address(state))} socket_pid=#{inspect(socket_pid)} code=#{
        inspect(code)
  ...
  defp handler_request(
    ...
    {_, ref} =
      spawn_monitor(fn ->
        response = handler.toku_request(request, codec)
  ...
  defp handler_push(%{handler: handler, codec: codec, handler_supports_push: true} = state, request) do
    spawn(handler, :toku_push, [request, codec])
    state
  end

  defp handler_terminate(%{handler: handler} = state, reason) do
    handler.toku_terminate(reason)
    state
  end
  ...
  defp set_opts(
    ...
    ee = select_enabled.(se, @default_codecs)

    ec =
      select_enabled.(sc, @default_compressors)
    ...
    %{
      state
      | ping_interval: ping_interval,
        supported_encodings: ee,
        supported_compressions: ec
    }
  end

  defp to_wire_format(%{codec: e, compressor: c}, data) do
    data
    |> e.encode()
    |> c.compress()
  end

  defp from_wire_format(%{codec: e, compressor: c}, data) do
    data
    |> c.decompress()
    |> e.decode()
  end
  ...
  defp choose_compressor(supported_compressions, [compression | compressions]) do
    case Map.get(supported_compressions, compression) do
      nil ->
        choose_codec(supported_compressions, compressions)
